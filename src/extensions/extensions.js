...
const timerStatus = (remainingMinutes) => {
  if (remainingMinutes === undefined) {
    return "You didn't set a timer!"
  }
return remainingMinutes === 0 ? `${"Phil's cake is ready!"}` : "The cake is still baking!"
}
...
const estimatePrepTime = (ingredients, prepTimeInMinutes=2) => {
...
const calculateQuantities = (ingredients, cakeLayers) => {
  const sugar = ingredients.includes('sugar') ? 100 * cakeLayers : 0
  const eggs = ingredients.includes('eggs') ? 2 * cakeLayers : 0
...
const improveRecipe = (ingredientInfo, numPortions) => {
  const updated = {}

  for (const ingredient in ingredientInfo) {
    updated[ingredient] = ingredientInfo[ingredient] * numPortions
  }
